...
add_filter( $hook, function ( $fields ) use ( $sep_style, $single_product_layout, $sidebars ) {
	...
	$args = array(
		...
		'product_featured_image' => array(
			'name'        => 'product_featured_image',
			'type'        => 'toggle',
			'settings'    => 'product_featured_image',
			'label'       => esc_html__( 'Featured image', 'xstore' ),
			'tooltip'     => esc_html__( 'Enable this option to display the featured image as the first image on individual product pages. When disabled, the gallery starts with its first image, while the featured image remains visible on product archive pages.', 'xstore' ),
			'section'     => 'single-product-page-layout',
			'default'     => 1,
		),
