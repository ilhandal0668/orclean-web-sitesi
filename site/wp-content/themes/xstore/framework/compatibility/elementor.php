...
add_action( 'elementor_pro/init', 'etheme_add_pdf_share_button_network', 20 );
if ( ! function_exists( 'etheme_add_pdf_share_button_network' ) ) {
	function etheme_add_pdf_share_button_network() {
		etheme_set_private_static_property(
			'ElementorPro\\Modules\\ShareButtons\\Module',
			'networks',
			function ( $networks ) {
				if ( ! is_array( $networks ) || isset( $networks['pdf'] ) ) {
					return $networks;
				}
		...
		etheme_add_pdf_share_button_icon_mapping();
	}
}

add_action( 'elementor/widgets/register', 'etheme_add_pdf_share_button_icon_mapping', 1 );
add_action( 'elementor/widgets/register', 'etheme_add_pdf_share_button_icon_mapping', 20 );
if ( ! function_exists( 'etheme_add_pdf_share_button_icon_mapping' ) ) {
	function etheme_add_pdf_share_button_icon_mapping() {
		etheme_set_private_static_property(
			'ElementorPro\\Modules\\ShareButtons\\Widgets\\Share_Buttons',
...
if ( ! function_exists( 'etheme_enqueue_product_pdf_assets' ) ) {
	function etheme_enqueue_product_pdf_assets() {
		if ( is_admin() || ! function_exists( 'is_product' ) || ! is_product() ) {
			return;
		}

		if ( ! wp_script_is( 'html2canvas', 'registered' ) ) {
			wp_register_script(
				'html2canvas',
				get_template_directory_uri() . '/js/libs/html2canvas.min.js',
				array(),
				'1.4.1',
				array(
					'in_footer' => true,
		...
		if ( ! wp_script_is( 'jspdf', 'registered' ) ) {
			wp_register_script(
				'jspdf',
				get_template_directory_uri() . '/js/libs/jspdf.umd.min.js',
				array(),
				'2.5.1',
				array(
					'in_footer' => true,
		...
		if ( ! wp_script_is( 'xstore-pdf-print-product', 'registered' ) ) {
			$script_path = get_template_directory() . '/js/modules/pdfPrintProduct.min.js';
			wp_register_script(
				'xstore-pdf-print-product',
				get_template_directory_uri() . '/js/modules/pdfPrintProduct.min.js',
		...
		wp_enqueue_script( 'xstore-pdf-print-product' );
	}
}

add_action( 'elementor/frontend/widget/before_render', 'etheme_enqueue_product_pdf_assets_for_elementor_share_buttons' );
if ( ! function_exists( 'etheme_enqueue_product_pdf_assets_for_elementor_share_buttons' ) ) {
	function etheme_enqueue_product_pdf_assets_for_elementor_share_buttons( $widget ) {
		$settings = array();

		if ( ! function_exists( 'is_product' ) || ! is_product() || ! is_object( $widget ) ) {
			return;
		}

		if ( ! method_exists( $widget, 'get_name' ) || 'share-buttons' !== $widget->get_name() ) {
			return;
		}
		...
		if ( empty( $settings['share_buttons'] ) || ! is_array( $settings['share_buttons'] ) ) {
			return;
		}

		foreach ( $settings['share_buttons'] as $button ) {
			if ( ! empty( $button['button'] ) && 'pdf' === $button['button'] ) {
				etheme_enqueue_product_pdf_assets();
				return;
			}
		}
	}
...
if ( ! function_exists( 'etheme_set_private_static_property' ) ) {
	function etheme_set_private_static_property( $class_name, $property_name, $callback ) {
		if ( ! class_exists( $class_name ) || ! is_callable( $callback ) ) {
			return false;
		}
		...
		try {
			...
			$property->setValue( null, call_user_func( $callback, $property->getValue() ) );
