...
if ( ! function_exists( 'etheme_search_post_excerpt' ) ) :
    ...
    function etheme_search_post_excerpt( $where = '' ) {
        ...
        if ( is_search() ) {
            ...
            if ( ! $is_product_query || empty( $wp_the_query->query_vars['s'] ) ) {
                return $where;
            }
...
add_filter( 'posts_search', 'etheme_search_parent_products_by_variation_sku', 999, 2 );
if ( ! function_exists( 'etheme_search_parent_products_by_variation_sku' ) ) :
    /**
...
add_filter( 'etheme_ajax_search_products_query', 'etheme_ajax_search_parent_products_by_variation_sku', 999 );
if ( ! function_exists( 'etheme_ajax_search_parent_products_by_variation_sku' ) ) :
    /**
     * Return parent products for XStore AJAX SKU search matches on product variations.
     */
    function etheme_ajax_search_parent_products_by_variation_sku( $args ) {
        if ( empty( $_REQUEST['query'] ) || empty( $args['meta_query'] ) || ! is_array( $args['meta_query'] ) ) {
            return $args;
        }

        if ( ! empty( $args['post_type'] ) && in_array( 'product_variation', (array) $args['post_type'], true ) ) {
            return $args;
        }
