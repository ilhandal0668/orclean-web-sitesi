...
if ( ! function_exists( 'etheme_template_hooks' ) ) {
	function etheme_template_hooks() {
		...
		if ( function_exists( 'is_product_category' ) && is_product_category() && etheme_get_option( 'category_breadcrumbs_below_banner', 0 ) && etheme_get_option( 'product_bage_banner_pos', 1 ) ) {
...
//		global $etheme_mini_cart_global;
		...
		if ( count($etheme_mini_cart_global[$cart_content_linked_products_type.'_ids']) ) {
			...
			$slider_args = array(
                ...
                'navigation_position_style' => $extra['navigation_position_style'],
				'navigation_position' => $extra['navigation_position'],
				'echo'            => false
			);
...
if ( ! function_exists( 'etheme_shop_filters_sidebar' ) ) {
	function etheme_shop_filters_sidebar() {
		...
		if ( is_active_sidebar( 'shop-filters-sidebar' ) ):
		    if ( apply_filters('etheme_should_enqueue_style', true) && apply_filters('etheme_filter_area_force_load_assets', true) ) {
...
if ( ! function_exists( 'etheme_product_countdown' ) ) {
	function etheme_product_countdown( $type = 'type2', $is_single = true ) {
		...
		echo ETC\App\Controllers\Shortcodes\Countdown::countdown_shortcode( array(
			...
			'month'  => date( 'M', $date ),
			'day'    => date( 'd', $date ),
			'hour'   => ( isset( $time_end[0] ) && $time_end[0] != 'Array' && $time_end[0] > 0 ) ? $time_end[0] : '23',
			'minute' => isset( $time_end[1] ) ? $time_end[1] : '59',
...
if ( ! function_exists( 'etheme_ajax_add_to_cart' ) ) {
	function etheme_ajax_add_to_cart() {
		// Get messages
		ob_start();
...
add_action( 'wp_ajax_etheme_ajax_add_to_cart', 'etheme_ajax_add_to_cart' );
add_action( 'wp_ajax_nopriv_etheme_ajax_add_to_cart', 'etheme_ajax_add_to_cart' );
...
if ( ! function_exists( 'etheme_get_mini_cart' ) ) {
	function etheme_get_mini_cart() {
        ...
        $custom_fragments = apply_filters('etheme_mini_cart_fragments', []);

        if ( count($custom_fragments) ) {
            $array['et_custom_fragments'] = $custom_fragments;
        }
		wp_send_json($array);
	}
}
...
if ( ! function_exists( 'etheme_should_show_single_product_featured_image' ) ) {
	/**
	 * Resolve featured image visibility for a single product gallery.
	 *
	 * @param WC_Product|int $product Product instance or ID.
	 *
	 * @return bool
	 */
	function etheme_should_show_single_product_featured_image( $product ) {
		...
		if ( $product instanceof WC_Product ) {
			...
			$visibility = get_post_meta( $product_id, ETHEME_PREFIX . 'featured_image_visibility', true );

			if ( ! in_array( $visibility, array( 'inherit', 'hidden', 'shown' ), true ) ) {
				// Products saved with the previous checkbox keep their hidden state.
				$visibility = get_post_meta( $product_id, ETHEME_PREFIX . 'hide_featured_image', true ) ? 'hidden' : 'inherit';
			}
		}
		...
		if ( 'shown' === $visibility ) {
			...
			$show_featured_image = (bool) etheme_get_option( 'product_featured_image', 1 );
		}

		return (bool) apply_filters(
			'etheme_single_product_featured_image_visible',
			$show_featured_image,
			$product,
			$visibility,
			$product_id
...
if ( ! function_exists( 'etheme_get_single_product_gallery_images' ) ) {
	/**
	 * Get the main and gallery image IDs for a single product gallery.
	 *
	 * @param WC_Product $product                    Product instance.
	 * @param bool       $allow_hide_featured_image Whether the product setting may hide the featured image.
	 *
	 * @return array{main_image_id:int, gallery_image_ids:int[], hide_featured_image:bool}
	 */
	function etheme_get_single_product_gallery_images( $product, $allow_hide_featured_image = true ) {
		$images = array(
			'main_image_id'       => 0,
		...
		$images['main_image_id']     = (int) $product->get_image_id();
		$images['gallery_image_ids'] = array_values( array_filter( array_map( 'absint', (array) $product->get_gallery_image_ids() ) ) );

		$hide_featured_image = ! etheme_should_show_single_product_featured_image( $product );
		if ( $allow_hide_featured_image && $hide_featured_image ) {
			$images['hide_featured_image'] = true;
			$images['main_image_id']       = $images['gallery_image_ids'] ? (int) array_shift( $images['gallery_image_ids'] ) : 0;
		}

		return apply_filters( 'etheme_single_product_gallery_images', $images, $product, $allow_hide_featured_image );
	}
}
			...
			if ( $view_mode_smart ) : ?>
				...
				<?php if ( $is_mobile ) : ?>
                    <div class="switch-grid <?php if ( $current == 'grid' ) { echo 'switcher-active'; } ?>">
                        <a data-type="grid" data-row-count="<?php echo esc_attr($current_view); ?>" rel="nofollow noopener" href="<?php echo esc_url( add_query_arg( 'et_columns-count', $current_view, remove_query_arg( 'et_columns-count', $url_grid ) ) ); ?>"></a>
                    </div>
				<?php else : ?>

                    <div class="switch-grid <?php if ( $current == 'grid' && $current_view == 2 ) { echo 'switcher-active'; } ?>">
                        <a data-type="grid" data-row-count="2" rel="nofollow noopener" href="<?php echo esc_url( add_query_arg( 'et_columns-count', '2', remove_query_arg( 'et_columns-count', $url_grid ) ) ); ?>"><?php esc_html_e( '2 columns grid', 'xstore' ); ?></a>
                    </div>

                    <div class="switch-grid <?php if ( $current == 'grid' && $current_view == 3 ) { echo 'switcher-active'; } ?>">
                        <a data-type="grid" data-row-count="3" rel="nofollow noopener" href="<?php echo esc_url( add_query_arg( 'et_columns-count', '3', remove_query_arg( 'et_columns-count', $url_grid ) ) ); ?>"><?php esc_html_e( '3 columns grid', 'xstore' ); ?></a>
                    </div>

                    <div class="switch-grid <?php if ( $current == 'grid' && $current_view == 4 ) { echo 'switcher-active'; } ?>">
                        <a data-type="grid" data-row-count="4" rel="nofollow noopener" href="<?php echo esc_url( add_query_arg( 'et_columns-count', '4', remove_query_arg( 'et_columns-count', $url_grid ) ) ); ?>"><?php esc_html_e( '4 columns grid', 'xstore' ); ?></a>
                    </div>
				
				<?php endif; ?>

                <div class="switch-list <?php if ( in_array('list', array($current, $current_view)) ) { echo 'switcher-active'; } ?>">
                    <a data-type="list" data-row-count="3" rel="nofollow noopener" href="<?php echo esc_url( $url_list ); ?>"><?php esc_html_e( 'List', 'xstore' ); ?></a>
                </div>
				...
				<?php if ( !$is_mobile ) : ?>
                    ...
                    <div class="switch-more <?php if ( in_array('more', array($current, $current_view)) ) { echo 'switcher-active'; } ?>">
                        ...
                        <ul>
                            <li class="<?php if ( $current == 'grid' && $current_view == 5 ) { echo 'switcher-active'; } ?>"><a data-type="grid" data-row-count="5" rel="nofollow noopener" href="<?php echo esc_url( add_query_arg( 'et_columns-count', '5', remove_query_arg( 'et_columns-count', $url_grid ) ) ); ?>"><?php esc_html_e( '5 columns grid', 'xstore' ); ?></a></li>
                            <li class="<?php if ( $current == 'grid' && $current_view == 6 ) { echo 'switcher-active'; } ?>"><a data-type="grid" data-row-count="6" rel="nofollow noopener" href="<?php echo esc_url( add_query_arg( 'et_columns-count', '6', remove_query_arg( 'et_columns-count', $url_grid ) ) ); ?>"><?php esc_html_e( '6 columns grid', 'xstore' ); ?></a></li>
                        </ul>
                    </div>
			...
			<?php else :
				if ( $view_mode == 'grid_list' ): ?>
                    <div class="switch-grid <?php if ( $current == 'grid' ) {
                        ...
                        <a data-type="grid" rel="nofollow noopener" href="<?php echo esc_url( $url_grid ); ?>"><?php esc_html_e( 'Grid', 'xstore' ); ?></a>
                    </div>
                    <div class="switch-list <?php if ( $current == 'list' ) {
						echo 'switcher-active';
					} ?>">
                        <a data-type="list" rel="nofollow noopener" href="<?php echo esc_url( $url_list ); ?>"><?php esc_html_e( 'List', 'xstore' ); ?></a>
                    </div>
...
if ( ! function_exists( 'etheme_woocommerce_sale_flash' ) ) {
	...
	function etheme_woocommerce_sale_flash( $span, $post, $product ) {
		...
		if ( strpos( $element_options['sale_label_text'], '%' ) !== false ) {
			...
			$element_options['sale_label_text'] = preg_replace( '/(\d+(?:[.,]\d+)?%)/', '<span>$1</span>', $element_options['sale_label_text'], 1 );
		}
		
		ob_start();
		
		if ( $element_options['show_label'] ) {
			echo '<div class="sale-wrapper ' . esc_attr( $element_options['wrapper_class'] ) . '"><span class="onsale ' . esc_attr( $element_options['class'] ) . '">' . wp_kses( $element_options['sale_label_text'], array( 'span' => array() ) ) . '</span></div>';
		}
...
if ( ! function_exists( 'etheme_available_variation_gallery' ) ):
	function etheme_available_variation_gallery( $available_variation, $variationProductObject, $variation ) {
		...
		$parent_gallery     = $parent_product instanceof WC_Product ? etheme_get_single_product_gallery_images( $parent_product ) : array(
			'main_image_id'       => 0,
			'gallery_image_ids'   => array(),
			'hide_featured_image' => false,
		);
		...
		if ( $has_variation_gallery_images ) {
			if ( '_wc_additional_variation_images' === $variation_gallery_images_meta_key ) {
		...
		$gallery_images = array_values( array_unique( array_filter( array_map( 'absint', (array) $gallery_images ) ) ) );

		if ( $parent_gallery['hide_featured_image'] && $parent_product->get_image_id() ) {
			$gallery_images = array_values( array_diff( $gallery_images, array( (int) $parent_product->get_image_id() ) ) );
		}
		...
		if ( $variation_image_id ) {
			...
			$gallery_images = $parent_gallery['gallery_image_ids'];

			if ( $parent_gallery['main_image_id'] ) {
				array_unshift( $gallery_images, $parent_gallery['main_image_id'] );
			}
		}
		...
		if ( ! $variation_image_id ) {
			$fallback_image_id               = ! empty( $gallery_images[0] ) ? (int) $gallery_images[0] : 0;
			$available_variation['image']    = wc_get_product_attachment_props( $fallback_image_id, $variation );
			$available_variation['image_id'] = $fallback_image_id;
		}
    ...
    <script type="text/html" id="tmpl-et-variation-gallery-slider-template">
        <div class="swiper-slide">
            <div class="woocommerce-product-gallery__image">
                <a data-elementor-open-lightbox="<?php echo !apply_filters('etheme_single_product_variation_gallery_lightbox', false) ? 'no' : 'yes' ?>" data-elementor-lightbox-slideshow="all-slide-<?php echo esc_attr($post->ID); ?>" href="{{data.url}}" data-large="{{data.full_src}}" data-width="{{data.full_src_w}}" data-height="{{data.full_src_h}}" data-index="{{data.index}}" class="woocommerce-main-image <# if (data.index < 1) { #> pswp-main-image <# } #> zoom">
...
function thumbnail_template_js() {
    ...
    <script type="text/html" id="tmpl-et-variation-gallery-thumbnail-template">
        <li class="<?php echo get_query_var('etheme_single_product_vertical_slider', false) ? 'slick-slide' : 'swiper-slide'; ?> thumbnail-item" <?php echo get_query_var('etheme_single_product_vertical_slider', false) ? 'style="width: 100%;"' : ''; ?>>
            <span data-elementor-open-lightbox="no" data-elementor-lightbox-slideshow="all-slide-<?php echo esc_attr($post->ID); ?>" href="{{data.url}}" data-small="{{data.src}}" data-large="{{data.full_src}}" data-width="{{data.full_src_w}}" data-height="{{data.full_src_h}}" class="pswp-additional pointer zoom" title="{{data.a_title}}">
...
if ( ! function_exists( 'et_get_default_variation_gallery_images' ) ):
	function et_get_default_variation_gallery_images( $product_id ) {
		...
		$gallery_images    = etheme_get_single_product_gallery_images( $product );
		$attachment_ids    = $gallery_images['gallery_image_ids'];
		$post_thumbnail_id = $gallery_images['main_image_id'];
...
function et_add_action_to_multi_currency_ajax( $ajax_actions ) {
    ...
    $ajax_actions[] = 'etheme_ajax_add_to_cart';
    $ajax_actions[] = 'etheme_update_cart_item_quantity';
	return $ajax_actions;
}
...
function etheme_show_single_stock(){
	...
	if (
        ...
        && !in_array($product->get_type(), array('variable', 'external', 'bundle'))
	){
...
add_action('init', function() {
	...
	add_filter( 'woocommerce_product_title', 'etheme_variation_title', 10, 2 );
		...
		add_action( 'woocommerce_product_query', function ( $q, $_this ) {
			if (
				$q instanceof WP_Query
				&& $q->is_main_query()
				&& $q->is_tax( 'product_cat' )
				&& apply_filters( 'etheme_disable_detached_variations_on_product_category_archive', true, $q, $_this )
			) {
				return $q;
			}
...
add_filter( 'woocommerce_product_categories_widget_args', function ( $args ) {
    if ( !apply_filters('etheme_widget_categories_advanced_mode', get_theme_mod('widget_product_categories_advanced_mode', 0)) || ! is_tax( 'product_cat' ) ) {
        return $args;
    }
...
function etheme_woocommerce_product_loop_start_filter($html) {
	...
	if ( !$woocommerce_loop['doing_ajax'] && apply_filters('etheme_should_enqueue_style', true) ) {
...
// Fix price filter for the search results page
add_action('woocommerce_product_query', function($query) {
    if (is_search() && $query->get('post_type') === 'product') {
        $meta_query = $query->get('meta_query') ?: [];
        // Apply min_price and max_price from the query vars
...
add_filter( 'woocommerce_ajax_variation_threshold', 'et_ajax_variation_threshold', 1001, 2 );
function et_ajax_variation_threshold( $threshold, $product ) {
    return 200;
}
 ...
 */
add_filter( 'woocommerce_login_redirect', 'etheme_woocommerce_force_proper_login_redirect', 999, 2 );
if ( !function_exists('etheme_woocommerce_force_proper_login_redirect') ) {
    function etheme_woocommerce_force_proper_login_redirect( $redirect, $user ) {
        if ( apply_filters('etheme_woocommerce_origin_login_redirect', false) )
            return $redirect;
...
add_filter( 'woocommerce_registration_redirect', 'etheme_woocommerce_force_proper_registration_redirect', 999 );
if ( !function_exists('etheme_woocommerce_force_proper_registration_redirect') ) {
    function etheme_woocommerce_force_proper_registration_redirect( $redirect ) {
        if ( apply_filters('etheme_woocommerce_origin_registration_redirect', false) )
            return $redirect;
