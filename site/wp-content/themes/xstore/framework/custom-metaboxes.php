	...
	function etheme_base_metaboxes() {
		...
		$tabs_setting['tabs']['layout'] = array(
			...
			'fields' => array(
				...
				array(
					'name'    => esc_html__( 'Featured image on single product page', 'xstore' ),
					'desc'    => esc_html__( 'Choose whether to inherit the Theme Options setting or override it for this product. The featured image remains visible on product archive pages.', 'xstore' ),
					'id'      => $prefix . 'featured_image_visibility',
					'type'    => 'select',
					'default_cb' => function ( $field_args, $field ) {
						return get_post_meta( $field->object_id(), ETHEME_PREFIX . 'hide_featured_image', true ) ? 'hidden' : 'inherit';
					},
					'options' => array(
						'inherit' => esc_html__( 'Inherit from Theme Options', 'xstore' ),
						'hidden'  => esc_html__( 'Hidden', 'xstore' ),
						'shown'   => esc_html__( 'Shown', 'xstore' ),
					),
				),
				array(
