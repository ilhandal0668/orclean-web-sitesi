...
if ( defined( 'DOING_AJAX' ) && DOING_AJAX && ! isset( $etheme_global['quick_view'] ) && !apply_filters('etheme_elementor_edit_mode', false) ) {
	...
	$gallery_images       = etheme_get_single_product_gallery_images( $product, ! isset( $etheme_global['quick_view'] ) );
	$attachment_ids       = $gallery_images['gallery_image_ids'];
	$main_attachment_id   = $gallery_images['main_image_id'];
	$hide_featured_image  = $gallery_images['hide_featured_image'];
}
...
?>
    <div <?php echo implode( ' ', $container_atts ); ?>>
        <div class="<?php echo esc_attr( $swiper_wrapper ); ?> main-images clearfix">
			...
			<?php if ( $main_attachment_id > 0 ) {

				$index             = 0;
				$columns           = apply_filters( 'woocommerce_product_thumbnails_columns', 4 );
				$post_thumbnail_id = $main_attachment_id;
				...
				if (!$full_size_image){
					...
					if ( $hide_featured_image ) {
						$attributes['class'] = 'attachment-woocommerce_single size-woocommerce_single wp-post-image';
					}
					...
					$html                 .= $hide_featured_image ? wp_get_attachment_image( $post_thumbnail_id, 'woocommerce_single', false, $attributes ) : get_the_post_thumbnail( $post->ID, 'woocommerce_single', $attributes );
					$html                 .= '</a>';

					echo '<div class="' . $gallery_slider_class . ' images woocommerce-product-gallery woocommerce-product-gallery__wrapper"><div data-thumb="' . esc_url( wp_get_attachment_image_url( $post_thumbnail_id, 'woocommerce_thumbnail' ) ) . '" class="' . $et_zoom_class . '">';
...
if ( $gallery_slider && $show_thumbs ) {
	...
	if ( empty( $attachment_ids ) && ! $hide_featured_image ) {
...
//					list( $magnifier_url, $magnifier_width, $magnifier_height ) = wp_get_attachment_image_src( $main_attachment_id, "shop_magnifier" );
					...
					if ( $image_link && $main_attachment_id > 0 ) {
