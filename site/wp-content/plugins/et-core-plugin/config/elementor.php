...
function etc_elementor_widgets_routes( $routes ) {
    ...
    if ( $is_pro_version ) {
        ...
        if ( apply_filters('etheme_elementor_search_results_pages_widget', false) ) {
            $routes[] = array(
                'ETC\App\Controllers\Elementor\Theme_Builder\Search_Results\Pages',
            );
        }
    }

    if ( $is_pro_version && $is_woocommerce ) {
        $routes[] = array(
            'ETC\App\Controllers\Elementor\Theme_Builder\Search_Results\Products',
        );
    }

    if ( $is_pro_version && $is_portfolio ) {
        $routes[] = array(
...
function etc_elementor_modules( $modules ) {
    ...
    $modules['container_stretch'] = array(
        'ETC\App\Controllers\Elementor\Modules\Container_Stretch'
    );
