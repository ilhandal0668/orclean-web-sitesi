...
//        );
		...
		$repeater->add_control(
			...
			[
				...
				'multiple' => true,
				'label_block' => true,
				'options' => $search->product_search_fields_options(),
        ...
        foreach ($settings['items'] as $item_index => $item) {
            ...
            switch ($item['element']) {
                ...
                case 'search':
					...
					$search_post_types = isset( $item['search_post_types'] ) ? (array) $item['search_post_types'] : array( 'product' );
                    $search_settings = array_merge($item, array(
                        'type' => 'inline',
                        'ajax_search' => $item['ajax_search'],
						'post_types' => $search_post_types,
						'product_search_fields_source' => isset( $item['product_search_fields_source'] ) ? $item['product_search_fields_source'] : 'global',
						'product_search_fields' => isset( $item['product_search_fields'] ) ? $item['product_search_fields'] : array( 'title', 'content', 'excerpt', 'sku' ),
                        'focus_overlay' => '',
...
//                        'post_types' => implode(',' )
                    ...
                    $search->render_inner_content($search_settings, $unique_id, array(
                        'form_attributes' => [
							...
							'data-post-types' => implode( ',', $search_post_types ),
                        ]
                    ));
