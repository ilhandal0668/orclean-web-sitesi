...
//                remove_filter('etheme_elementor_theme_builder', '__return_true');
            ...
            if ( $products && $products->have_posts() ) {
                ...
                if ( $settings['query_type'] == 'search_query' ) {
					...
					if ( is_search() && isset( $_GET['et_search_fields'] ) && function_exists( 'etheme_get_product_search_fields' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
						self::$page_link = add_query_arg(
							'et_search_fields',
							implode( ',', etheme_get_product_search_fields( wp_unslash( $_GET['et_search_fields'] ) ) ), // phpcs:ignore WordPress.Security.NonceVerification.Recommended
							self::$page_link
						);
					}
                }
...
//                $query_args['order']    = 'DESC';
	...
	public function get_content_product($settings, $edit_mode = 'undefined') {
        ...
        <div <?php wc_product_class( $class, $product ); ?><?php if ($local_settings['image_hover_effect'] == 'slider') :
            echo 'data-images="'.$this->etheme_get_image_list($product, true, ($local_settings['image_size'] != 'custom' ? $local_settings['image_size'] : 'woocommerce_thumbnail' ) ).'"';
