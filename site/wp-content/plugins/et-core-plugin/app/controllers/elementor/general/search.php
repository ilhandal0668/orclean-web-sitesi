...
class Search extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'product_search_fields_source',
			[
				'label' => esc_html__( 'Product Search Fields', 'xstore-core' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'options' => [
					'global' => esc_html__( 'Use global settings', 'xstore-core' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Search In', 'xstore-core' ),
				'type' => \Elementor\Controls_Manager::SELECT2,
				'multiple' => true,
				'label_block' => true,
				'options' => self::product_search_fields_options(),
				'default' => [ 'title', 'content', 'excerpt', 'sku' ],
				'conditions' => [
					'relation' => 'and',
					'terms' => [
						[
						...
						[
							'name' => 'product_search_fields_source',
							'operator' => '==',
							'value' => 'custom',
						],
					],
...
//				],
    ...
    public function render_inner_content($settings, $unique_id, $extra_settings = array()) {
        ...
        $product_search_fields = self::get_product_search_fields_for_display( $settings );

        $this->add_render_attribute(
            'form'.$unique_id, [
                'class' => 'etheme-search-form',
                'role' => 'search',
                'action' => apply_filters('etheme_search_form_action_url', home_url()),
                'method' => 'get',
                'type' => $settings['type']
            ]
        );
		...
		if ( $product_search_fields ) {
			$this->add_render_attribute(
				'form' . $unique_id,
				'data-product-search-fields',
				implode( ',', $product_search_fields )
			);
		}
        ...
        <form <?php echo $this->get_render_attribute_string( 'form'.$unique_id ); ?>>
            ...
            <div class="etheme-search-input-form-wrapper">
                ...
                <div class="etheme-search-input-wrapper">
					...
					<?php if ( $product_search_fields ) : ?>
						<input type="hidden" name="et_search_fields" value="<?php echo esc_attr( implode( ',', $product_search_fields ) ); ?>">
					<?php endif; ?>
    ...
    public static function product_search_fields_options() {
        return [
            'title'   => esc_html__( 'Product title', 'xstore-core' ),
            'content' => esc_html__( 'Product description', 'xstore-core' ),
            'excerpt' => esc_html__( 'Short description', 'xstore-core' ),
            'sku'     => esc_html__( 'SKU', 'xstore-core' ),
            'tags'    => esc_html__( 'Product tags', 'xstore-core' ),
        ];
    }

    /**
     * Return a widget-specific product search field list.
     *
     * An empty array means that the global Customizer setting should be used.
     */
    public static function get_product_search_fields_for_display( $settings ) {
        if ( 'custom' !== ( isset( $settings['product_search_fields_source'] ) ? $settings['product_search_fields_source'] : 'global' ) ) {
            return array();
        }

        $fields = isset( $settings['product_search_fields'] ) ? $settings['product_search_fields'] : array( 'title' );

        if ( function_exists( 'etheme_get_product_search_fields' ) ) {
            return etheme_get_product_search_fields( $fields );
        }

        $allowed_fields = array_keys( self::product_search_fields_options() );
        $fields         = is_array( $fields ) ? array_filter( $fields, 'is_string' ) : explode( ',', (string) $fields );
        $fields         = array_map( 'sanitize_key', $fields );
        $fields         = array_values( array_unique( array_intersect( $allowed_fields, $fields ) ) );

        return $fields ? $fields : array( 'title' );
    }
    ...
    public function render_trending_searches_list($tags) {
		...
		$product_search_fields = self::get_product_search_fields_for_display( $this->get_settings_for_display() );

		if ( $product_search_fields ) {
			$query_args['et_search_fields'] = implode( ',', $product_search_fields );
		}
