...
if ( ! function_exists( 'add_action' ) ) {
	...
	// phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.wp_remote_get_wp_remote_get -- vip_safe_wp_remote_get() only exists on WordPress VIP; this plugin ships to self-hosted stores.
	$woo_feed_challan_api_response = wp_remote_get(
		$woo_feed_challan_url,
		array(
			'timeout'   => 20, // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- api.wordpress.org is slow to answer plugin_information; the 20s budget is the shipped V5 behaviour and this is an admin-only, on-demand page render.
			'sslverify' => false,
		)
	);

	$woo_feed_challan_response = wp_remote_retrieve_body( $woo_feed_challan_api_response );
	...
	// phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.wp_remote_get_wp_remote_get -- vip_safe_wp_remote_get() only exists on WordPress VIP; this plugin ships to self-hosted stores.
	$woo_feed_disco_api_response = wp_remote_get(
		$woo_feed_disco_url,
		array(
			'timeout'   => 20, // phpcs:ignore WordPressVIPMinimum.Performance.RemoteRequestTimeout.timeout_timeout -- api.wordpress.org is slow to answer plugin_information; the 20s budget is the shipped V5 behaviour and this is an admin-only, on-demand page render.
			'sslverify' => false,
	...
	$woo_feed_disco_response = wp_remote_retrieve_body( $woo_feed_disco_api_response );
	...
	/**
	 * Calculate the weighted average star rating from a wp.org ratings breakdown.
	 *
	 * @since 8.0.0
	 *
	 * @param array $ratings Ratings breakdown keyed 1-5 with the vote count for each star.
	 *
	 * @return float|int Weighted average rounded to 2 decimals, or 0 when there are no votes.
	 */
	function woo_feed_calculate_rating( $ratings ) {
		$totalRatings = array_sum( $ratings ); // Sum of all ratings.
		// phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual -- array_sum() returns int or float depending on the decoded wp.org payload; a strict check would miss the 0.0 case.
		if ( 0 == $totalRatings ) {
			return 0;
		}
	...
	if ( ! function_exists( 'woo_feed_is_plugin_activated' ) ) {
		/**
		 * Check whether one of the promoted WebAppick plugins is active.
		 *
		 * @since 8.0.0
		 *
		 * @param string $plugin_slug wp.org plugin slug, e.g. 'disco'.
		 *
		 * @return bool True when the plugin's main file is active.
		 */
		function woo_feed_is_plugin_activated( $plugin_slug ) {

			if ( 'webappick-pdf-invoice-for-woocommerce' === $plugin_slug ) {
...
<section class="woo_feed_our_plugin_container">
	...
	<img class="our_plugin_plus_1" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/plus-icon-1.png" alt="<?php esc_attr_e( 'Add Plus logo', 'woo-feed' ); ?>">

	<img class="our_plugin_plus_2" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/plus-icon-2.png" alt="<?php esc_attr_e( 'Add Plus logo', 'woo-feed' ); ?>">

	<img class="our_plugin_circle_2" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/circle-2.png" alt="<?php esc_attr_e( 'Add circle icon 2', 'woo-feed' ); ?>">

	<img class="our_plugin_circle_1" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/circle-1.png" alt="<?php esc_attr_e( 'Add circle icon 2', 'woo-feed' ); ?>">
...
<!-------------------------- our_plugin_card_two part start  -------------------->
		...
		<span class="dashicons dashicons-star-filled"></span>
		<?php
		// phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual -- woo_feed_calculate_rating() returns a float from round(); a strict check against the integer 5 could never match.
		if ( 5 == $woo_feed_disco_rating ) {
		...
		<img class="our_plugin_kon_icon" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/kon-icon.png" alt="<?php esc_attr_e( 'Add circle icon 2', 'woo-feed' ); ?>">
		</div>
	...
	<section class="woo_feed_about_us_images">
		...
		<img class="about_us_img" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/about-us.png" alt="<?php esc_attr_e( 'Add Disco logo in our_plugin page', 'woo-feed' ); ?>">
		</div>
		</div>


		<img class="our_plugin_circle_3" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/circle-3.png" alt="<?php esc_attr_e( 'Add Plus logo', 'woo-feed' ); ?>">

		<img class="our_plugin_multiplication" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/multiplication.png" alt="<?php esc_attr_e( 'Add Plus logo', 'woo-feed' ); ?>">

		<img class="our_plugin_dottet" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/dottet.png" alt="<?php esc_attr_e( 'Add Plus logo', 'woo-feed' ); ?>">

		<img class="our_plugin_kon_2" src="<?php echo esc_url( WOO_FEED_PLUGIN_URL ); ?>V8/Admin/images/our_plugins/kon-2.png" alt="<?php esc_attr_e( 'Add Plus logo', 'woo-feed' ); ?>">
	</section>
