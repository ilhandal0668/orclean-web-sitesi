...
use CTXFeed\Compat\Support\CommonHelper;
...
class WPSEO_FrontendCompatibility {
	 ...
	 * @return string
	 */
	public function primary_category( $primary_category, $product, $config ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter, VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Hook callback signature: the arity registered with add_filter()/add_action() must be preserved even though this adapter does not read every argument.
		if ( ! empty( $primary_category ) ) {
			return $primary_category;
		}

		if ( ! class_exists( 'WPSEO_Frontend' ) || ! function_exists( 'yoast_get_primary_term_id' ) ) {
			return $primary_category;
		}
	 ...
	 * @return string
	 */
	public function wpseo_title( $title, $product, $config ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter, VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Hook callback signature: the arity registered with add_filter()/add_action() must be preserved even though this adapter does not read every argument.
	 ...
	 * @return string
	 */
	public function wpseo_metadesc( $description, $product, $config ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter, VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Hook callback signature: the arity registered with add_filter()/add_action() must be preserved even though this adapter does not read every argument.
		if ( ! class_exists( 'WPSEO_Frontend' ) ) {
			return $description;
		}
	 ...
	 * @return string
	 */
	public function canonical_url( $canonical_url, $product, $config ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter, VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Hook callback signature: the arity registered with add_filter()/add_action() must be preserved even though this adapter does not read every argument.
